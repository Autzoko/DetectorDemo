"""
Single-case inference: run detection on one NIfTI file.

Handles data conversion, preprocessing, model inference, and post-processing
for a single input file. Supports optional supplementary spatial data if
available alongside the input file.

Usage:
    python run_single.py --input /path/to/scan.nii.gz
    python run_single.py --input /path/to/scan.nii.gz --no_tta
    python run_single.py --input /path/to/scan.nii.gz --output_dir ./my_results
"""
...
def _index_spatial_data(nii_path, tar_path, cache_dir):
...
def main():
    ...
    # Use a temporary data directory for this single case
    ...
    case_id = prepare_single_case(nii_path, tmp_data)
    ...
    if tar_path:
        _index_spatial_data(nii_path, tar_path, cache_dir)
    ...
    # Update config to point to temp data dir
    ...
    for old_pkl in pred_dir.glob("case_00000_*.pkl"):
        old_pkl.unlink()
